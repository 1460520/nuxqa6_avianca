...
export type TBookingPage = {
    ...
    selectFlightOutbound(copyBooking): Promise<void>;
    selectFlightReturn(copyBooking): Promise<void>;
    validateModalFlight(): Promise<void>;
    continueToPassenger(copyBooking): Promise<void>;
    editFlightSelected(copyBooking): Promise<void>;
...
const BookingPage: TBookingPage = {
    ...
    async selectFlightReturn(copyBooking): Promise<void> {
        ...
        try {
            ...
            //nuxqa6
            await page.waitForSelector(".fare_button");
            await page.locator('.fare_button').first().click({ delay: helper.getRandomDelay() });
    ...
    async selectFlightOutbound(copyBooking): Promise<void> {
    ...
    async editFlightSelected(copyBooking): Promise<void> {
    ...
    async continueToPassenger(copyBooking): Promise<void> {
        ...
        try {
            ...
            if(copyBooking.consulta_condiciones_tarifa){
                ...
                await expect(newTab).toHaveURL("https://nuxqa6.avtest.ink/es/informacion-y-ayuda/tarifas-avianca/");
                newTab.close();
            }
            await page.waitForSelector(".button.page_button.btn-action.page_button-primary-flow");
            await expect(page.locator(".button.page_button.btn-action.page_button-primary-flow")).toBeVisible();
            await page.locator(".button.page_button.btn-action.page_button-primary-flow").click({ delay: helper.getRandomDelay() });
             await page.waitForTimeout(3500);
            // const buttonConfirmResumen = page.locator(".button.page_button.btn-action.page_button-primary-flow");
    ...
    async run(data): Promise<void> {
        ...
        if(data.numero_vuelo_ida){
            await this.selectFlightOutbound(data);
            await this.validateModalFlight();
            await page.waitForTimeout(3500);
            if(data.numero_vuelo_regreso){
                await this.selectFlightReturn(data);
                await this.validateModalFlight();
                if(data.consulta_condiciones_tarifa){
                    await helper.takeScreenshot("resumen-seleccion-vuelos");
                    await this.continueToPassenger(data);
                }   
            }
        }
